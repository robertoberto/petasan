...
def start_all_disks():
    ...
    for disk_meta in disks:
        if not is_disk_started(disk_meta.id):
            if start_disk(disk_meta.id,disk_meta.pool):
                print "Started disk " + disk_meta.id + " " + disk_meta.disk_name
            else:
                print "Error starting disk " + disk_meta.id + " " + disk_meta.disk_name
    return True
