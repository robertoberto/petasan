...
function set_cancel_attach(path) {
    $("#cancelBtn").click(function () {
        window.location = path;
    });
}

function set_slider_attach(size_list) {
    $(document).ready(function () {
        $(function () {
            ...
            $("#diskSizeVal").on('change', function (e) {
...
$(document).ready(function () {
    ...
    $("#diskName").focus();
    //show and hide elemnt as default selected
...
$("#attach_task_form").submit(function (event) {
    ...
    // check not empty
    if (!diskSizeValue) {
        $("#lblDiskSize").text(messages.attach_disk_lbl_size_empty);
    ...
    // check is digit
        ...
        $("#lblDiskSize").text(messages.attach_disk_lbl_size_invalid);
        diskSize.closest('.form-group').addClass('has-error');
        diskSize.focus();
        event.preventDefault();
    }
    ...
    // check not empty
    if (!diskNameValue) {
        $("#lblDiskName").text(messages.attach_disk_lbl_name_empty);
    ...
    // check length
        ...
        $("#lblDiskName").text(messages.attach_disk_lbl_name_length);
        diskName.closest('.form-group').addClass('has-error');
        diskName.focus();
        event.preventDefault();
    }
    ...
    if (ISCSIsubnetValue == 0) {
        $("#lblISCSISubnet").text(messages.attach_disk_lbl_subnet_empty);
    ...
    if (!$('#orpUseFirstRangeYes').is(':checked')) {
        ...
        if (!ValidateIPaddress($("#path1Val").val())) {
            $("#lblPath1").text(messages.attach_disk_lbl_path1_invalid);
        ...
        if ($("#ActivePaths").val() == 2) {
            ...
            if (!ValidateIPaddress($("#path2Val").val())) {
                $("#lblPath2").text(messages.attach_disk_lbl_path2_invalid);
    ...
    if (!$('#orpAclAll').is(':checked')) {
        ...
        // check not empty
        if (!iqnValue) {
            $("#lblACL").text(messages.attach_disk_lbl_iqn_empty);
        ...
        if (iqnValue.toLowerCase().indexOf(",") >= 0) {
            ...
            for (i = 0; i < values.length; i++) {
                if (!values[i].match(/^(?:iqn\.[0-9]{4}-[0-9]{2}(?:\.[a-z](?:[a-z0-9\-]*[a-z0-9])?)+(?::.*)?)$/)) {
                    ...
                    $("#lblACL").text(messages.attach_disk_lbl_iqn_invalid);
                    $("#lblACL").closest('.form-group').addClass('has-error');
                    iqn.closest('.form-group').addClass('has-error');
                    iqn.focus();
                    event.preventDefault();
    ...
    if (!$('#orpAuthNo').is(':checked')) {
        ...
        // check not empty
        if (!userNameValue) {
            $("#lblUserName").text(messages.attach_disk_lbl_username_empty);
        ...
        // check length
            ...
            $("#lblUserName").text(messages.attach_disk_lbl_username_length);
            userName.closest('.form-group').addClass('has-error');
            userName.focus();
            event.preventDefault();
        }
        ...
        // check not empty
        if (!passwordValue) {
            $("#lblPass").text(messages.attach_disk_lbl_password_empty);
        ...
        // check length
            ...
            $("#lblPass").text(messages.attach_disk_lbl_password_length);
            password.closest('.form-group').addClass('has-error');
            password.focus();
            event.preventDefault();
        }
        ...
        if (!confirmPasswordValue) {
            $("#lblConfirmPass").text(messages.attach_disk_lbl_confirm_password_empty);
        ...
        // check length
            ...
            $("#lblConfirmPass").text(messages.attach_disk_lbl_confirm_password_mismatch);
            confirmPassword.closest('.form-group').addClass('has-error');
            confirmPassword.focus();
            event.preventDefault();
        }
